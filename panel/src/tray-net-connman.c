...
/*
 * tray-net-connman.c — D-Bus client for ConnMan
 */
#include "tray-net.h"
        ...
        if (dbus_message_get_args(msg, NULL,
            ...
            if (strcmp(name, CONNMAN_SERVICE) == 0) {
                if (new_owner && new_owner[0]) {
                    ...
                    fprintf(stderr, "isde-panel: tray-net: ConnMan disappeared\n");
...
int tn_connman_init(TrayNet *tn)
{
    ...
    if (!tn->system_bus) {
        fprintf(stderr, "isde-panel: tray-net: system bus: %s\n",
    ...
    if (dbus_error_is_set(&err)) {
        fprintf(stderr, "isde-panel: tray-net: D-Bus match: %s\n", err.message);
        dbus_error_free(&err);
    }
...
int tn_connman_get_technologies(TrayNet *tn)
{
    ...
    while (dbus_message_iter_get_arg_type(&array) == DBUS_TYPE_STRUCT) {
        if (tn->ntechs >= TN_MAX_TECHNOLOGIES)
            break;
...
int tn_connman_get_services(TrayNet *tn)
{
    ...
    while (dbus_message_iter_get_arg_type(&array) == DBUS_TYPE_STRUCT) {
        if (tn->nservices >= TN_MAX_SERVICES)
            break;
...
static void async_reply_cb(DBusPendingCall *pending, void *user_data)
{
    ...
    if (reply) {
        if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
            ...
            fprintf(stderr, "isde-panel: tray-net: %s failed: %s\n",
...
int tn_connman_tech_set_powered(TrayNet *tn, const char *path, int powered)
{
    ...
    if (!reply) {
        if (dbus_error_is_set(&err)) {
            fprintf(stderr, "isde-panel: tray-net: set powered: %s\n", err.message);
            dbus_error_free(&err);
        }
        return -1;
    }
