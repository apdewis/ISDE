...
/*
 * tray-bt-bluez.c — D-Bus client for BlueZ
 *
...
static void parse_device_properties(BtDeviceInfo *d, DBusMessageIter *dict)
{
    while (dbus_message_iter_get_arg_type(dict) == DBUS_TYPE_DICT_ENTRY) {
        DBusMessageIter entry, variant;
        dbus_message_iter_recurse(dict, &entry);
...
static void handle_properties_changed(TrayBt *tb, DBusMessage *msg)
{
    ...
    if (strcmp(iface, BLUEZ_DEVICE_IFACE) == 0) {
        BtDeviceInfo *dev = tb_bluez_find_device(tb, path);
...
static void handle_interfaces_added(TrayBt *tb, DBusMessage *msg)
{
    ...
    while (dbus_message_iter_get_arg_type(&ifaces) == DBUS_TYPE_DICT_ENTRY) {
        ...
        if (strcmp(iface, BLUEZ_DEVICE_IFACE) == 0 && path_is_device(path)) {
            if (tb->ndevices < TB_MAX_DEVICES &&
                !tb_bluez_find_device(tb, path)) {
                BtDeviceInfo *d = &tb->devices[tb->ndevices];
                memset(d, 0, sizeof(*d));
        ...
        if (dbus_message_get_args(msg, NULL,
            ...
            if (strcmp(name, BLUEZ_SERVICE) == 0) {
                if (new_owner && new_owner[0]) {
                    ...
                    fprintf(stderr, "isde-panel: tray-bt: BlueZ disappeared\n");
...
int tb_bluez_init(TrayBt *tb)
{
    ...
    if (!tb->system_bus) {
        fprintf(stderr, "isde-panel: tray-bt: system bus: %s\n",
    ...
    if (dbus_error_is_set(&err)) {
        fprintf(stderr, "isde-panel: tray-bt: D-Bus match: %s\n", err.message);
        dbus_error_free(&err);
    }
...
int tb_bluez_set_powered(TrayBt *tb, int powered)
{
    ...
    if (!reply) {
        if (dbus_error_is_set(&err)) {
            fprintf(stderr, "isde-panel: tray-bt: set powered: %s\n",
                    err.message);
            dbus_error_free(&err);
        }
        return -1;
...
static void async_reply_cb(DBusPendingCall *pending, void *user_data)
{
    ...
    if (reply) {
        if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
            ...
            fprintf(stderr, "isde-panel: tray-bt: %s failed: %s\n",
...
static void device_op_reply_cb(DBusPendingCall *pending, void *user_data)
{
    ...
    if (reply) {
        if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
            BtDeviceInfo *dev = tb_bluez_find_device(tb, op->path);
            if (dev)
                dev->busy = 0;
            ...
            isde_dialog_message(tb->panel->toplevel, "Bluetooth", buf,
                                NULL, NULL);
            tb_menu_rebuild(tb);
        }
        dbus_message_unref(reply);
...
int tb_bluez_device_trust(TrayBt *tb, const char *path)
{
    ...
    if (!reply) {
        if (dbus_error_is_set(&err)) {
            fprintf(stderr, "isde-panel: tray-bt: trust device: %s\n",
                    err.message);
            dbus_error_free(&err);
        }
        return -1;
...
int tb_bluez_device_remove(TrayBt *tb, const char *path)
{
    ...
    if (!reply) {
        if (dbus_error_is_set(&err)) {
            fprintf(stderr, "isde-panel: tray-bt: remove device: %s\n",
                    err.message);
            dbus_error_free(&err);
        }
        return -1;
