...
/*
 * tray-bt-agent.c — BlueZ pairing agent for isde-panel
 *
...
agent_message_handler(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    ...
    if (strcmp(member, "RequestPinCode") == 0) {
        ...
        tb->agent_dialog = isde_dialog_input(
            tb->panel->toplevel, "Bluetooth Pairing", prompt, "",
    ...
    if (strcmp(member, "DisplayPasskey") == 0) {
        ...
        tb->agent_dialog = isde_dialog_message(
            tb->panel->toplevel, "Bluetooth Pairing", message,
    ...
    if (strcmp(member, "RequestConfirmation") == 0) {
        ...
        tb->agent_dialog = isde_dialog_confirm(
            tb->panel->toplevel, "Bluetooth Pairing", message, "Confirm",
...
int tb_agent_init(TrayBt *tb)
{
    ...
    if (!dbus_connection_register_object_path(tb->system_bus,
                                               BLUEZ_AGENT_PATH,
                                               &agent_vtable, tb)) {
        fprintf(stderr, "isde-panel: tray-bt: failed to register agent path\n");
        return -1;
    }
    ...
    if (!reply) {
        if (dbus_error_is_set(&err))
            fprintf(stderr, "isde-panel: tray-bt: RegisterAgent: %s\n",
                    err.message);
        dbus_error_free(&err);
        return -1;
    }
    ...
    if (msg) {
        ...
        if (!reply) {
            if (dbus_error_is_set(&err))
                fprintf(stderr, "isde-panel: tray-bt: RequestDefaultAgent: %s\n",
                        err.message);
            dbus_error_free(&err);
        } else {
            dbus_message_unref(reply);
    ...
    dbus_error_free(&err);
    fprintf(stderr, "isde-panel: tray-bt: agent registered\n");
    return 0;
}
