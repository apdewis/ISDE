...
/*
 * tray-mount-password.c — passphrase entry dialog for LUKS devices
 *
...
void tm_password_dialog_show(TrayMount *tm, int device_idx)
{
    ...
    MountDeviceInfo *d = &tm->devices[device_idx];
    snprintf(tm->pw_dev_path, sizeof(tm->pw_dev_path), "%s", d->dev_path);

    tm->pw_shell = isde_dialog_input(
        tm->panel->toplevel, "Unlock Encrypted Volume", "Enter passphrase", "",
        on_input_result, tm);
