...
/*
 * tray-net-agent.c — ConnMan Agent for PSK passphrase entry
 */
#include "tray-net.h"
...
agent_message_handler(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    ...
    if (strcmp(member, "ReportError") == 0) {
        ...
        dbus_message_get_args(msg, NULL,
                              ...
                              DBUS_TYPE_INVALID);
        fprintf(stderr, "isde-panel: tray-net: agent error: %s: %s\n",
    ...
    if (strcmp(member, "RequestInput") == 0) {
        ...
        tn->agent_dialog = isde_dialog_input(
            tn->panel->toplevel, "WiFi Passphrase", prompt, "",
            on_input_result, tn);
...
int tn_agent_init(TrayNet *tn)
{
    ...
    if (!dbus_connection_register_object_path(tn->system_bus,
                                               CONNMAN_AGENT_PATH,
                                               &agent_vtable, tn)) {
        fprintf(stderr, "isde-panel: tray-net: failed to register agent path\n");
        return -1;
    }
    ...
    if (!reply) {
        if (dbus_error_is_set(&err))
            fprintf(stderr, "isde-panel: tray-net: RegisterAgent: %s\n", err.message);
        dbus_error_free(&err);
        return -1;
    }
    ...
    dbus_message_unref(reply);
    dbus_error_free(&err);
    fprintf(stderr, "isde-panel: tray-net: agent registered\n");
    return 0;
}
