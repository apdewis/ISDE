...
/*
 * tray-mount-dbus.c — D-Bus client for the mount tray module
 *
...
signal_filter(DBusConnection *conn, DBusMessage *msg, void *user_data)
{
    ...
    if (strcmp(member, "DeviceAdded") == 0) {
        ...
        if (dbus_message_get_args(msg, NULL,
                                   ...
                                   DBUS_TYPE_INVALID)) {
            if (tm->ndevices < TM_MAX_DEVICES) {
                MountDeviceInfo *d = &tm->devices[tm->ndevices++];
                memset(d, 0, sizeof(*d));
            ...
            fprintf(stderr, "isde-panel: tray-mount: device added: %s\n",
                    dev_path);
        }
        ...
        if (dbus_message_get_args(msg, NULL,
            ...
            fprintf(stderr, "isde-panel: tray-mount: device removed: %s\n",
                    dev_path);
        }
...
int tm_dbus_init(TrayMount *tm)
{
    ...
    if (!tm->system_bus) {
        fprintf(stderr, "isde-panel: tray-mount: D-Bus system bus: %s\n",
    ...
    if (dbus_error_is_set(&err)) {
        fprintf(stderr, "isde-panel: tray-mount: D-Bus match: %s\n",
                err.message);
        dbus_error_free(&err);
    }
    ...
    fprintf(stderr, "isde-panel: tray-mount: connected to system bus\n");
    dbus_error_free(&err);
    return 0;
}
...
int tm_dbus_list_devices(TrayMount *tm)
{
    ...
    if (!reply) {
        if (dbus_error_is_set(&err)) {
            fprintf(stderr, "isde-panel: tray-mount: ListDevices: %s\n",
                    err.message);
            dbus_error_free(&err);
        }
        return -1;
...
static void on_op_reply(DBusPendingCall *pending, void *user_data)
{
    ...
    if (!reply) {
        fprintf(stderr, "isde-panel: tray-mount: no reply for %s\n",
                ctx->dev_path);
    ...
    if (success) {
        fprintf(stderr, "isde-panel: tray-mount: %s OK: %s %s\n",
                op_name, ctx->dev_path, res_str);
    } else {
        fprintf(stderr, "isde-panel: tray-mount: %s failed: %s\n",
                op_name, res_str);
        isde_dialog_message(tm->panel->toplevel, "Error", res_str,
                            NULL, NULL);
    }
