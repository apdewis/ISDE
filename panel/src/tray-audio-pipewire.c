...
/*
 * tray-audio-pipewire.c — PipeWire integration for the audio tray module
 *
                       ...
                       const char *message)
{
    ...
    fprintf(stderr, "isde-panel: tray-audio: PipeWire error %d: %s\n",
            res, message);
}
...
int ta_pw_init(TrayAudio *ta)
{
    ...
    if (!ta->pw_main_loop) {
        fprintf(stderr, "isde-panel: tray-audio: pw_main_loop_new failed\n");
        return -1;
    }
    ...
    if (!ta->pw_context) {
        fprintf(stderr, "isde-panel: tray-audio: pw_context_new failed\n");
    ...
    if (!ta->pw_core) {
        fprintf(stderr, "isde-panel: tray-audio: pw_context_connect failed\n");
    ...
    if (!ta->pw_registry) {
        fprintf(stderr, "isde-panel: tray-audio: pw_core_get_registry failed\n");
    ...
    if (fd >= 0) {
        IswAppAddInput(ta->panel->app, fd, (IswPointer)IswInputReadMask,
                       pw_input_cb, ta);
    }
